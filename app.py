...
def update_match_charts(selected_team, matchday_range):
    ...
    results["label"] = results["home_team_name"].str[:3] + " v " + results["away_team_name"].str[:3]
    ...
    results_fig.add_trace(go.Scatter(
        ...
        marker=dict(
            size=8,
            color=results["total"],
            colorscale=[[0, COLOURS["accent2"]], [0.5, COLOURS["accent3"]], [1, COLOURS["danger"]]],
            showscale=True,
            colorbar=dict(title="Goals", thickness=12, len=0.8),
        ),
        text=results["label"],
        hovertemplate="<b>%{text}</b><br>Matchday %{x}<br>%{y} goals<extra></extra>",
    ))
